 ...
 */
export async function searchRecipesByIngredients(ingredients, number = 10, ranking = 1, ignorePantry = true) {
 ...
 */
export async function getRecipeDetails(recipeId, includeNutrition = false) {
    if (!API_KEY || API_KEY === 'your_api_key_here') {
        throw new Error('API key not configured. Please add your Spoonacular API key to .env.local');
    }
    try {
 ...
 */
export async function searchRecipes(query, number = 10, cuisine, diet, intolerances, includeIngredients) {
    if (!API_KEY || API_KEY === 'your_api_key_here') {
    ...
    try {
        ...
        if (intolerances)
            url.searchParams.append('intolerances', intolerances);
