...
import type { OcrDateResult } from '../types'
...
export async function extractDateFromImage(file: File): Promise<OcrDateResult> {
  const { data } = await Tesseract.recognize(file, 'fra+eng', { logger: _m => {} })
  const text = (data.text || '').toLowerCase()

  type Cand = { iso: string; kind: 'DLC'|'DDM'|'UNKNOWN'; score: number; ts: number; raw: string }
  const cands: Cand[] = []

  const hintScore = (DLC_HINTS.some(h=>text.includes(h))?2:0)+(DDM_HINTS.some(h=>text.includes(h))?1:0)
  const kind: 'DLC'|'DDM'|'UNKNOWN' =
    DLC_HINTS.some(h=>text.includes(h)) ? 'DLC' :
